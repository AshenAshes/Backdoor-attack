...
def main():
    ...
    # for index in [0]:
        ...
        train_data_dirty = MyDataset(train_data, 0, mask=1, portion=0.1*index, mode="train", device=device)
        train_data_clean = MyDataset(train_data, 0, mask=1, portion=0, mode="train", device=device)

        test_data_orig = MyDataset(test_data, 0, mask=1, portion=0, mode="train", device=device)
        test_data_trig = MyDataset(test_data, 0, mask=1, portion=1, mode="test", device=device)
    ...
    x = [0, 10, 20, 30, 40, 50]
    print(distance)
    plt.title("dirty rate - distance")
    plt.plot(x, distance, label="alpha=")
