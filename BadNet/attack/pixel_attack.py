...
def attack_success(x, img, target_calss, model, targeted_attack=False, verbose=False):
...
def attack(img, label, model, target=None, pixels=1, maxiter=75, popsize=400, verbose=False):
    # img: 1*3*W*H tensor
    # label: a number
    ...
    predict_fn = lambda xs: predict_classes(
        xs, img, target_calss, model, target is None)
    callback_fn = lambda x, convergence: attack_success(
        x, img, target_calss, model, targeted_attack, verbose)
